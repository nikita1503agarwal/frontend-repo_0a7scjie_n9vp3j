...
// Separate component so hooks aren't called inside a loop/map
function VisualBox({ product }) {
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* floating gradient plane to give depth (use transform, not translateZ prop) */}
          <motion.div
            className="absolute -inset-6 rounded-[40px] blur-3xl"
            style={{ background: `radial-gradient(60% 80% at 70% 30%, ${product.colors?.[1] || '#2563eb'}40 0%, transparent 70%)`, transform: 'translateZ(-40px)' }}
            aria-hidden
          />
          ...
          <motion.div
            ...
            style={{
              background: `conic-gradient(from 0deg, transparent, ${product.colors?.[0] || '#111827'}44, transparent 70%)`,
              mixBlendMode: 'screen',
